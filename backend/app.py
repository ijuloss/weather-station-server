...
class Config:
    ...
    def load_config(self):
        ...
        for cfg_path, label in config_files:
            if cfg_path.exists():
                try:
                    self._apply_kv_map(dotenv_values(str(cfg_path)), source_label=label)
...
def setup_logging():
    ...
    return logging.getLogger("weather-station")
...
if firebase_config.get('databaseURL') and firebase_config.get('apiKey'):
    ...
    firebase_last_ok = firebase_initialized
    if firebase_initialized:
        logger.info("SUKSES: Firebase terhubung menggunakan REST API")
    else:
        logger.warning("PERINGATAN: Koneksi Firebase gagal - menjalankan mode lokal")
else:
    logger.info("INFO: Firebase tidak dikonfigurasi - menjalankan mode lokal")
...
def receive_sensor_data():
    ...
    try:
        ...
        if sensor_reading.get('device_id'):
            last_esp32_device_id = sensor_reading.get('device_id')
    ...
    try:
        ...
        logs = [
            ...
            },
            {
                "timestamp": iso_now(),
                "level": "INFO", 
                "message": "WiFi connected to WeatherStation_Network"
            },
            {
                "timestamp": iso_now(),
                "level": "DEBUG",
                "message": "Sensor reading: T=25.5°C, H=60.0%, AQ=150ppm"
            }
        ]
