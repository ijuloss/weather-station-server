...
class WeatherAIModel:
    ...
    def save_model(self):
        """Save model to disk secara atomic."""
        try:
    ...
    def train_model(self, force_single_class: bool = False):
        ...
        try:
            ...
            if len(local_data) < 50:
                ...
                logger.error(msg)
                self.last_metrics = {"status": status, "message": msg}
                return False
            ...
            if len(unique_labels) < 2 and not force_single_class:
                ...
                logger.error(msg)
                self.last_metrics = {"status": status, "message": msg, "label_distribution": dict(label_counts)}
                return False

            if len(unique_labels) < 2 and force_single_class:
                logger.warning("AI: Force training diaktifkan, menambah sampel sintetik (minimal 2/kelas). Evaluasi NON_VALID.")
                ...
                sample_data.extend(archetypes * 2)
                X, y, timestamps = self.prepare_training_data(sample_data)
                unique_labels = np.unique(y)
                label_counts = Counter(y)
                if len(unique_labels) < 2:
                    status = "DATA_INSUFFICIENT_VARIETY"
                    msg = "AI: Force training gagal, tetap 1 kelas meski sudah ditambah sintetik."
                    logger.error(msg)
                    self.last_metrics = {"status": status, "message": msg, "label_distribution": dict(label_counts)}
                    return False
            ...
            def _parse_ts(ts_raw, idx):
                try:
                    if ts_raw is None:
                        return datetime.fromtimestamp(idx, tz=timezone.utc)
            ...
            # Baseline majority class (mengacu train)
            train_counts_for_baseline = Counter(y_train)
            majority_label, majority_count = train_counts_for_baseline.most_common(1)[0]
            baseline_acc = majority_count / len(y_test) if len(y_test) > 0 else 0.0
            ...
            logger.info(
                "AI: Metrics train_acc=%.4f test_acc=%.4f macro_f1=%.4f bal_acc=%.4f baseline_majority=%.4f mode=%s",
                train_score, test_score, macro_f1, bal_acc, baseline_acc, evaluation_mode
            )
...
def train_ai_model():
    ...
    try:
        ...
        def _run_train():
            global ai_training_in_progress
            try:
                weather_ai.train_model(force_single_class=force_single_class)
            finally:
                ai_training_in_progress = False
