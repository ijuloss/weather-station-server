...
<<<<<<< HEAD
  ...
  // --- 1. WiFi Signal Strength Icon (Left, 16x14 pixels) ---
  int rssi = WiFi.isConnected() ? WiFi.RSSI() : -100;
  const uint8_t* wifiIcon = icon_wifi_0;
  if (rssi > -55) wifiIcon = icon_wifi_3;       // Excellent
  else if (rssi > -70) wifiIcon = icon_wifi_2;  // Good
  else if (rssi > -85) wifiIcon = icon_wifi_1;  // Fair
  // else icon_wifi_0 (No signal / weak)
  u8g2.drawXBMP(0, 1, 16, 14, wifiIcon);

  // --- 2. SSID Display with Scrolling Text ---
  u8g2.setFont(u8g2_font_5x7_tf);
  String ssid = WiFi.isConnected() ? WiFi.SSID() : "No WiFi";
  int ssidW = u8g2.getStrWidth(ssid.c_str());
  const int ssidBoxX = 18;
  const int ssidBoxW = 42;
  
  if (ssidW > ssidBoxW) {
    // Scrolling animation for long SSID
    if (now - g_lastSsidScrollMs > 80) {
      g_ssidScrollX--;
      if (g_ssidScrollX < -(ssidW + 10)) g_ssidScrollX = ssidBoxW;
      g_lastSsidScrollMs = now;
    }
    u8g2.setClipWindow(ssidBoxX, 0, ssidBoxX + ssidBoxW, 14);
...
<<<<<<< HEAD
  ...
  if (!g_lastPostOk && (now % 1000 < 500)) {
    u8g2.drawLine(64, 1, 83, 12);
    u8g2.drawLine(83, 1, 64, 12);
  }

  // --- 4. Battery Icon with Charging Indicator (Right side) ---
  // Battery frame: 16x12 horizontal style
  const int batX = 96;
  const int batY = 2;
  const int batW = 18;
  const int batH = 10;
  
  // Draw battery outline (horizontal orientation)
  u8g2.drawFrame(batX, batY, batW - 2, batH);         // Main body
  u8g2.drawBox(batX + batW - 2, batY + 3, 2, 4);      // Battery tip (right)
  
  // Calculate battery fill level
  int pct = getBattPct(g_lastReading.battery_voltage);
  int fillW = ((batW - 6) * pct) / 100;
  if (fillW > batW - 6) fillW = batW - 6;
  if (fillW < 0) fillW = 0;
  
  // Fill the battery body
  if (fillW > 0) {
    u8g2.drawBox(batX + 2, batY + 2, fillW, batH - 4);
  }

  // Lightning bolt overlay when CHARGING (current_mA > 0)
  // Note: Negative current means discharging, positive means charging
  bool isCharging = (isfinite(g_lastReading.current_mA) && g_lastReading.current_mA > 5.0f);
  if (isCharging) {
    // Draw small bolt in center of battery
    u8g2.setDrawColor(2); // XOR mode for visibility
    u8g2.drawXBMP(batX + 5, batY + 1, 6, 10, icon_bolt_small);
    u8g2.setDrawColor(1);
  }
  
  // Battery percentage text (next to or under battery)
  u8g2.setFont(u8g2_font_4x6_tf);
  char pctStr[5];
  snprintf(pctStr, sizeof(pctStr), "%d%%", pct);
  int pctW = u8g2.getStrWidth(pctStr);
  // Position: right of battery or below if no space
  u8g2.drawStr(batX + ((batW - pctW) / 2), batY + batH + 6, pctStr);

  // --- Separator Line ---
  u8g2.drawHLine(0, 19, 128);

  // ===================== PAGE CONTENT (Below status bar) =====================
  const int cY = 22;

  switch (g_oledPage) {
    case 0: // PAGE 1: ENVIRONMENT (2x2 Grid)
    {
      // --- Top Left: Temperature ---
      u8g2.drawXBMP(2, cY, 16, 16, icon_temp_16x16);
      u8g2.setFont(u8g2_font_6x12_tr);
      u8g2.setCursor(20, cY + 12);
      if (isfinite(g_lastReading.temperature)) {
        u8g2.print(g_lastReading.temperature, 1);
        u8g2.print("\260C");
      } else { 
        u8g2.print("--"); 
      }
    ...
    case 1: // PAGE 2: GPS & LOCATION
    {
      ...
      // GPS Status
      u8g2.setCursor(0, cY + 8);
      u8g2.print("GPS     : ");
      u8g2.print(g_lastReading.gps_valid ? "Fix" : "No Fix");
      
      // Latitude
      u8g2.setCursor(0, cY + 18);
      u8g2.print("Lintang : ");
      if (g_lastReading.gps_valid) {
        u8g2.print(g_lastReading.lat, 6);
      } else {
        u8g2.print("-");
      }
      
      // Longitude
      u8g2.setCursor(0, cY + 28);
      u8g2.print("Bujur   : ");
      if (g_lastReading.gps_valid) {
        u8g2.print(g_lastReading.lng, 6);
      } else {
        u8g2.print("-");
      }
      
      // Location (static placeholder or from reverse geocode)
      u8g2.setFont(u8g2_font_5x7_tf);
      u8g2.setCursor(0, cY + 38);
      u8g2.print("Lokasi  : Jalan Sidotopo Lor I,");
      u8g2.setCursor(0, cY + 45);
      u8g2.print("          Surabaya, Jawa Timur");
      break;
    }

    case 2: // PAGE 3: POWER / INA219 Data
    {
      u8g2.setFont(u8g2_font_6x12_tr);
      
      // Voltage
      u8g2.setCursor(0, cY + 12);
      u8g2.print("Tegangan : ");
      if (isfinite(g_lastReading.bus_voltage)) {
        u8g2.print(g_lastReading.bus_voltage, 2);
        u8g2.print(" V");
      } else {
        u8g2.print("-- V");
      }
      
      // Current (show sign to indicate charge/discharge)
      u8g2.setCursor(0, cY + 26);
      u8g2.print("Arus     : ");
      if (isfinite(g_lastReading.current_mA)) {
        u8g2.print(g_lastReading.current_mA, 0);
        u8g2.print(" mA");
      } else {
        u8g2.print("-- mA");
      }
      ...
      // Power
      u8g2.setCursor(0, cY + 40);
      u8g2.print("Daya     : ");
      if (isfinite(g_lastReading.power_mW)) {
        u8g2.print(g_lastReading.power_mW, 0);
        u8g2.print(" mW");
      } else {
        u8g2.print("-- mW");
      }
    ...
    case 3: // PAGE 4: SYSTEM / NETWORK INFO
    {
      u8g2.setFont(u8g2_font_6x10_tf);
      
      // Web address
      u8g2.setCursor(0, cY + 10);
      u8g2.print("Web  : ws.ijuloss.my.id");
      
      // IP Address
      u8g2.setCursor(0, cY + 22);
      u8g2.print("IP   : ");
      if (WiFi.isConnected()) {
      ...
      // HTTP Status Code
      u8g2.setCursor(0, cY + 34);
      u8g2.print("HTTP : ");
      if (g_lastPostCode > 0) {
        u8g2.print(g_lastPostCode);
        // Add status description
        if (g_lastPostCode == 200) u8g2.print(" OK");
        else if (g_lastPostCode == 400) u8g2.print(" Bad");
        else if (g_lastPostCode == 500) u8g2.print(" Err");
        else if (g_lastPostCode == 504) u8g2.print(" Timeout");
      } else {
        u8g2.print("---");
      }
=======
